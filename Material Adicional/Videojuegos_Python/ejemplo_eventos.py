...
class EventosTeclado(cocos.layer.ColorLayer):
    ...
    def __init__(self):
        ...
        self.texto = cocos.text.Label('', x = 30, y = 20,
...
class EventosRaton(cocos.layer.Layer):
    ...
    def on_mouse_scroll(self, x, y, scroll_x, scroll_y):
        if scroll_y == 1.0:
            self.actualiza_texto(x, y, 'Se ha movido la rueda'
                                       ' del ratón hacia adelante')
        elif scroll_y == -1.0:
            self.actualiza_texto(x, y, 'Se ha movido la rueda'
